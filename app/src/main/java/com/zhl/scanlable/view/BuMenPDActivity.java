...
import com.zhl.scanlable.bean.BuMenBean;
...
public class BuMenPDActivity extends AppBaseActivity {
    ...
    List<BuMenBean.DataBean> cangkBean = new ArrayList<>();
    BuMenBean.DataBean cangKData;
...
//       将属性设置给窗体
    ...
    public void newCangKDialog() {
        ...
        //初始化控件
        RecyclerView recyclerView = inflate.findViewById(R.id.rv_list);
        cangKAdapter = new BaseQuickAdapter<BuMenBean.DataBean, BaseViewHolder>(R.layout.item_type, cangkBean) {

            @Override
            protected void convert(BaseViewHolder helper, BuMenBean.DataBean item) {
                helper.setText(R.id.tv_type, item.getDepartmentName());
            }
        };
        cangKAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                tvCk.setText(cangkBean.get(position).getDepartmentName());
                cangKData = cangkBean.get(position);
                depotId = cangKData.getDepartmentId();
                cangKDialog.dismiss();
            }
        });
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
...
//                goodsListAdapter.notifyDataSetChanged();
    ...
    protected void initEvent() {
        OkgoBaseModel.getInstance().getRequestNetGET(this, Constant.allDepartment, null, new ICallListener<String>() {
            @Override
            public void callSuccess(String s) {
                Gson gson = new Gson();
                BuMenBean cangKBean = gson.fromJson(s, BuMenBean.class);
    ...
    public void requestNet(final ArrayList<String> list) {
        ...
        map.put("departmentId", depotId);
        LogUtil.e(signalSource);
        bean1OkgoBaseModel.getRequestNetGET(this, Constant.getGoodsByRfidAndgoodsTypeIdAndDepartmentId, map, new ICallListener<String>() {
            @Override
            public void callSuccess(String s) {
                LogUtil.e(s);
                Gson gson = new Gson();
...
//						UHfData.Inventory_6c_Mask((byte)0, 16, 0, UHfGetData.hexStringToBytes("E200"));
    ...
    public void onViewClicked() {
        ...
        map.put("departmentId", cangKData.getDepartmentId());
        OkgoBaseModel.getInstance().getRequestNetGET(this, Constant.getGoodsCountDepartment, map, new ICallListener<String>() {
            @Override
            public void callSuccess(String s) {
                Gson gson = new Gson();
