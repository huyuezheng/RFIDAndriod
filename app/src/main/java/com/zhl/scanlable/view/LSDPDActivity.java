...
import com.zhl.scanlable.bean.LSDbean;
...
public class LSDPDActivity extends AppBaseActivity {
    ...
    List<LSDbean.DataBean> cangkBean = new ArrayList<>();
    LSDbean.DataBean cangKData;
...
//       将属性设置给窗体
    ...
    public void newCangKDialog() {
        ...
        //初始化控件
        RecyclerView recyclerView = inflate.findViewById(R.id.rv_list);
        cangKAdapter = new BaseQuickAdapter<LSDbean.DataBean, BaseViewHolder>(R.layout.item_type, cangkBean) {

            @Override
            protected void convert(BaseViewHolder helper, LSDbean.DataBean item) {
                helper.setText(R.id.tv_type, item.getSaleShopName());
            }
        };
        cangKAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                tvCk.setText(cangkBean.get(position).getSaleShopName());
                cangKData = cangkBean.get(position);
                depotId = cangKData.getSaleShopId();
                cangKDialog.dismiss();
            }
        });
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
...
//                goodsListAdapter.notifyDataSetChanged();
    ...
    protected void initEvent() {
        OkgoBaseModel.getInstance().getRequestNetGET(this, Constant.allSaleShop, null, new ICallListener<String>() {
            @Override
            public void callSuccess(String s) {
                Gson gson = new Gson();
                LSDbean cangKBean = gson.fromJson(s, LSDbean.class);
    ...
    public void requestNet(final ArrayList<String> list) {
        ...
        map.put("saleShopId", depotId);
        LogUtil.e(signalSource);
        bean1OkgoBaseModel.getRequestNetGET(this, Constant.getGoodsByRfidAndgoodsTypeIdAndSaleShopId, map, new ICallListener<String>() {
            @Override
            public void callSuccess(String s) {
                LogUtil.e(s);
                Gson gson = new Gson();
...
//						UHfData.Inventory_6c_Mask((byte)0, 16, 0, UHfGetData.hexStringToBytes("E200"));
    ...
    public void onViewClicked() {
        ...
        map.put("saleShopId", cangKData.getSaleShopId());
        OkgoBaseModel.getInstance().getRequestNetGET(this, Constant.getGoodsCountSaleShop, map, new ICallListener<String>() {
            @Override
            public void callSuccess(String s) {
                Gson gson = new Gson();
